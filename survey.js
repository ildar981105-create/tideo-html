...
var testMode=params.get('smode')||params.get('mode'),testRound=params.get('test')||'default',inviteMode=params.get('invite')==='1'||params.get('public')==='1',uid=params.get('uid')||(inviteMode&&global.TideoTracker&&global.TideoTracker.uid)||((inviteMode)?getInviteUid(testRound):'');
function getInviteUid(test){var key='tideo_invite_uid_'+encodeURIComponent(test||'default');try{var saved=localStorage.getItem(key);if(!saved){saved='guest_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,8);localStorage.setItem(key,saved)}return saved}catch(e){return'guest_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,8)}}
